...
# These points define a distorted quadrilateral, not a rectangle
...
# Draw dummy text lines on the distorted paper to visualize internal warp correction
...
# Target points: Flattened, rectangular top-down perspective (defining the final output size)
# We map the 4 distorted points to a perfectly rigid 300x400 rectangle starting at (0,0)
out_w, out_h = 300, 400
pts_dst = np.float32([
    [0, 0],          # Top-Left target
...
# This matrix defines the mapping between pts_src and pts_dst
H = cv2.getPerspectiveTransform(pts_src, pts_dst)

# Apply the 3D-to-2D perspective warp to the entire image
# The output size is specified by (out_w, out_h)
...
print("="*60)
# Homography matrices have 8 degrees of freedom; H[2,2] is usually normalized to 1.
